...
VALUES('D'||DINING_RES_SEQ.NEXTVAL, 42, 'testda2', 2, 3, SYSDATE, '12:00', 1, 2, 2); 
...
SELECT RNUM, DR.RES_NO CHECK_REVIEW, B.DINING_NAME DN, TO_CHAR(DE.RES_DATE, 'YYYY/MM/DD')TRANSDATE, DE.* 
FROM (SELECT ROWNUM AS RNUM, D.* FROM(SELECT * FROM DINING_RES WHERE MEMBER_ID = 'admin' ORDER BY RES_NO DESC )D)DE 
JOIN DINING B ON (DE.DINING_NO = B.DINING_NO) 
LEFT OUTER JOIN DINING_REVIEW DR ON(DE.RES_NO = DR.RES_NO)
WHERE RNUM BETWEEN 0 AND 10 ORDER BY LPAD(DE.RES_NO,4,'0');

SELECT TO_CHAR(DE.RES_DATE, 'YYYY/MM/DD')TRANSDATE, DE.* 
...
LEFT OUTER JOIN DINING_REVIEW DR ON(DE.RES_NO = DR.RES_NO)
WHERE RNUM BETWEEN 0 AND 10 AND MEMBER_ID='admin' ORDER BY LPAD(DE.RES_NO,4,'0');
